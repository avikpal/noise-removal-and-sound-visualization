/*
 *  Copyright 2013-2014  Avik Pal<avikpal.me@gmail.com>
 *
...
*/
  ...
  if(argc==3 || argc==2){
    ...
    if(strcmp(argv[1], "record")==0){ //start recording
       ...
       */
      if(argc==2){
	system("gst-launch alsasrc ! audioconvert ! audioresample ! vorbisenc ! oggmux ! filesink location= SampleTest.ogg");}
      
      if(argc==3){
      /* Save the recorded audio file with the name provided by user 
       */
	
	strcpy(CommandString,"gst-launch alsasrc ! audioconvert ! audioresample ! vorbisenc ! oggmux ! filesink location=");
  ...
  else printf("Error in input formatting");
}
